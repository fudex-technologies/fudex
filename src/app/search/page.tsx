...
export default function SearchPage() {
	...
	const categoryIds = search.cat && search.cat.length > 0 ? search.cat : undefined;
	...
	const hasSearchQuery = !!search.q || (categoryIds && categoryIds.length > 0);
						...
						<SectionWrapper className='w-full flex flex-col gap-5'>
							...
							{vendors.length === 0 && products.length === 0 && (
								<p className='text-foreground/50 text-center py-8'>
									{search.q 
										? `No results found for "${search.q}"${categoryIds && categoryIds.length > 0 ? ' in selected categories' : ''}`
