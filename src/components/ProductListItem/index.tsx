...
const ProductListItem = ({
	...
	const image = productItem?.images && productItem?.images?.length > 0
	...
	const description = productItem?.description || productItem?.product?.description || '';

	const productId = productItem?.productId || productItem?.id;

	// Build URL with slug as query parameter if available
	const baseUrl = PAGES_DATA.single_vendor_product_page(productItem?.vendorId, productId);
		...
		<Link
			...
			<div className='flex-1 flex justify-between flex-wrap gap-x-2 py-1'>
				<div className='flex flex-col'>
					<p className=''>{shortenText(`${productItem?.product?.name} (${productItem?.name})`, 30)}</p>
					{description && (
						<p className='text-foreground/50 text-sm'>{shortenText(description, 50)}</p>
					)}
				</div>
				<p className=' text-sm'>From {formatCurency(productItem?.price)}</p>
			</div>
